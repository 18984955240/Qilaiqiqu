 ...
 *  单聊
 */
public class ChatSingleActivity extends MyBaseActivity {
	...
	protected void addTextToList(String type, String text, int who,
		...
		map.put("type", type);
		map.put("person", who);
		map.put("image", who == ME ? SystemUtil.IMGPHTH_NEW + imgPath
				: SystemUtil.IMGPHTH_NEW + imgPath);
		map.put("text", text);
		map.put("name", name);
	...
	private class MyChatAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			System.out.println("chatList.get(position).get(from[0]).toString()="+chatList.get(position).get(from[0]).toString());
			SystemUtil.Imagexutils_new(chatList.get(position).get(from[0]).toString(),holder.imageView, context);
			...
			if (chatList.get(position).get("type").toString().equals("TXT")) {
				...
				if (who == OTHER) {
					System.out.println("chatList.get(position).get(from[2]).toString()="+chatList.get(position).get(from[2]).toString());
					SystemUtil.Imagexutils_new(chatList.get(position).get(from[2]).toString(),holder.textImgView, context);
					/*c.with(context)
